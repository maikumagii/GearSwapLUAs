...
function handle_job_elemental(command, target)
    ...
    elseif command:contains('skillchain') then
        ...
        if command == 'skillchain' then
            ...
            local skillchain1_uses_helix = skillchain1.first_spell:lower():contains('helix') or
            skillchain1.second_spell:lower():contains('helix')
            local skillchain2_uses_helix = skillchain2.first_spell:lower():contains('helix') or
            skillchain2.second_spell:lower():contains('helix')
        ...
        elseif last_character == '3' then
            ...
            elseif (get_current_stratagem_count() + immactive) < 3 then
                ...
                elseif spell_recasts[283] > (spell_latency + 16.3 * 60) then
                    add_to_chat(123,
                        'Abort: [Pyrohelix] waiting on recast. (' .. seconds_to_clock(spell_recasts[283] / 60) .. ')')
                else
                    if not state.Buff['Immanence'] then windower.chat.input('/ja "Immanence" <me>') end
        ...
        elseif command == 'endskillchain' then
            ...
            if spell_recasts[second_spell_id] > (spell_latency + 6.3 * 60) then
                ...
                windower.chat.input:schedule(1.3,
                    '/p {' ..
                    skillchain.skillchain .. '} -' .. player.target.name ..
                    '- MB: ' .. state.ElementalMode.value .. ' CLOSE!')
                windower.chat.input:schedule(1.3,
                    '/ma "' .. data.elements.helix_of[state.ElementalMode.value] .. 'helix" ' .. target)
            end
        end
        return true
    end
