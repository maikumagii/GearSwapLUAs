-- Local SCH overrides extracted from data/SCH.lua so the base job file can be
-- resynced from upstream with fewer merge conflicts.
...
function job_setup()
    ...
    data.skillchains.scholar.fallback = {}
    data.skillchains.scholar.fallback['1'] = {
    ...
    data.skillchains.scholar.fallback['2'] = {
    ...
    init_job_states(
    { "Capacity", "AutoFoodMode", "AutoTrustMode", "AutoWSMode", "AutoNukeMode", "AutoShadowMode", "AutoStunMode",
        "AutoDefenseMode" },
        { "AutoBuffMode", "AutoRuneMode", "Weapons", "OffenseMode", "WeaponskillMode", "IdleMode", "Passive",
    ...
    if not spell.interrupted then
        ...
        elseif spell.action_type == 'Magic' then
            if state.UseCustomTimers.value and spell.english == 'Sleep' or spell.english == 'Sleepga' then
                windower.send_command('@timers c "' ..
...
update_active_strategems = update_active_stratagems

function get_sch_skillchain_timing(skillchain)
    ...
    return first_spell_delay, second_immanence_delay, close_delay
end

function get_sch_skillchain_recast_block(skillchain, spell_recasts)
    ...
    local first_spell_delay, _, close_delay = get_sch_skillchain_timing(skillchain)

    if spell_recasts[first_spell_id] > (spell_latency + first_spell_delay * 60) then
        return 'Abort: [' ..
            skillchain.first_spell ..
            '] waiting on recast. (' .. seconds_to_clock(spell_recasts[first_spell_id] / 60) .. ')'
    elseif spell_recasts[second_spell_id] > (spell_latency + close_delay * 60) then
        return 'Abort: [' ..
            skillchain.second_spell ..
            '] waiting on recast. (' .. seconds_to_clock(spell_recasts[second_spell_id] / 60) .. ')'
    end

    return nil
end

function select_sch_skillchain(spell_recasts)
    local element = state.ElementalMode.value
    local candidates = {
        data.skillchains.scholar['2'][element],
        data.skillchains.scholar.fallback['2'][element],
        data.skillchains.scholar['1'][element],
        data.skillchains.scholar.fallback['1'][element],
    }
    local first_block

    for _, skillchain in ipairs(candidates) do
        local recast_block = get_sch_skillchain_recast_block(skillchain, spell_recasts)
        if not recast_block then
            return skillchain
        elseif not first_block then
            first_block = recast_block
        end
    end

    add_to_chat(123, first_block)
    return nil
end
...
function handle_job_elemental(command, target)
    ...
    elseif command:contains('skillchain') then
        ...
        if command == 'skillchain' or last_character == '1' or last_character == '2' then
            local spell_recasts = windower.ffxi.get_spell_recasts()
            local skillchain = command == 'skillchain' and select_sch_skillchain(spell_recasts) or
                data.skillchains.scholar[last_character][state.ElementalMode.value]

            if skillchain then
                local first_spell_delay, second_immanence_delay, close_delay = get_sch_skillchain_timing(skillchain)
                local recast_block = get_sch_skillchain_recast_block(skillchain, spell_recasts)

                if recast_block then
                    add_to_chat(123, recast_block)
                    return true
                end

                if not state.Buff['Immanence'] then windower.chat.input('/ja "Immanence" <me>') end
                windower.chat.input('/p {' ..
                skillchain.skillchain .. '} -' .. player.target.name .. '- MB: ' .. skillchain.burst_elements .. ' OPEN!')
                windower.chat.input:schedule(first_spell_delay, '/ma "' .. skillchain.first_spell ..
                '" ' .. player.target.id)
                windower.chat.input:schedule(second_immanence_delay, '/ja "Immanence" <me>')
                windower.chat.input:schedule(close_delay,
                    '/p {' ..
                    skillchain.skillchain .. '} -' .. player.target.name ..
                    '- MB: ' .. skillchain.burst_elements .. ' CLOSE!')
        ...
        elseif last_character == '3' then
            ...
            elseif (get_current_stratagem_count() + immactive) < 3 then
                ...
                if spell_recasts[159] > (spell_latency + 1.3 * 60) then
                    add_to_chat(123, 'Abort: [Stone] waiting on recast. (' ..
                    seconds_to_clock(spell_recasts[159] / 60) .. ')')
                elseif spell_recasts[281] > (spell_latency + 6.9 * 60) then
                    add_to_chat(123,
                        'Abort: [Pyrohelix] waiting on recast. (' .. seconds_to_clock(spell_recasts[281] / 60) .. ')')
                elseif spell_recasts[283] > (spell_latency + 16.3 * 60) then
                    add_to_chat(123,
                        'Abort: [Ionohelix] waiting on recast. (' .. seconds_to_clock(spell_recasts[283] / 60) .. ')')
                else
                    if not state.Buff['Immanence'] then windower.chat.input('/ja "Immanence" <me>') end
                    ...
                    windower.chat.input:schedule(6.9, '/p {Liquefaction} -' .. player.target.name ..
                    '- MB: {Fire} CLOSE!')
                    windower.chat.input:schedule(6.9, '/ma "Pyrohelix" ' .. player.target.id)
                    windower.chat.input:schedule(13, '/ja "Immanence" <me>')
                    windower.chat.input:schedule(16.3,
                        '/p {Fusion} -' .. player.target.name .. '- MB: {Fire}, {Light} CLOSE!')
        ...
        elseif command == 'endskillchain' then
            local spell_recasts = windower.ffxi.get_spell_recasts()
            local helix_spell = data.elements.helix_of[state.ElementalMode.value] .. 'helix'
            local helix_id = get_spell_id_by_name(helix_spell)

            if spell_recasts[helix_id] > (spell_latency + 1.3 * 60) then
                add_to_chat(123,
                    'Abort: [' .. helix_spell .. '] waiting on recast. (' ..
                    seconds_to_clock(spell_recasts[helix_id] / 60) .. ')')
            else
                if not state.Buff['Immanence'] then windower.chat.input('/ja "Immanence" <me>') end
                windower.chat.input:schedule(1.3,
                    '/p {Skillchain} -' .. player.target.name .. '- MB: ' .. state.ElementalMode.value .. ' CLOSE!')
                windower.chat.input:schedule(1.3, '/ma "' .. helix_spell .. '" ' .. target)
            end
        elseif command == 'wsskillchain' then
            if player.tp < 1000 then
                ...
                windower.chat.input:schedule(6.3,
                    '/p {' ..
                    skillchain.skillchain .. '} -' .. player.target.name ..
                    '- MB: ' .. skillchain.burst_elements .. ' CLOSE!')
